...
public class View {
    ...
    protected void runMove() { // TODO
    	if(this.model.getCurrentAgent() != null && !(this.model.isHumanTurn())) {
    		this.model.move(this.model.getCurrentAgent());
    		this.boardPanel.updateCells();
    	}
    	
    	else if(this.boardPanel.toCell != null && this.boardPanel.fromCell != null){
    		this.model.move(new Move(this.boardPanel.fromCell, this.boardPanel.toCell));
    		this.boardPanel.updateCells();
    	}
    	...
    	else if(this.model.getBoard().getTurn() == Piece.Type.MUSKETEER) {
    ...
    private void saveBoard() { // TODO
    	File[] f = new File("boards").listFiles();
    	
    	for(File file: f) {
    		if(this.saveFileNameTextField.getText().endsWith(".txt") && (this.saveFileNameTextField.getText() != file.getName())) {
    			this.model.getBoard().saveBoard(new File("boards/"+this.saveFileNameTextField.getText()));
    		}
    	}
    }
